...
def min_conflicts(csp, max_steps=100000):
    ...
    csp.current = current = {} #tạo một danh sách các Current ( các giá trị giả định )
    for var in csp.variables:
        val = min_conflicts_value(csp, var, current) # gán các giá trị vào các biến trong current
        csp.assign(var, val, current) # cập nhật current vào assignment
    
    for i in range(max_steps): # chạy vòng lặp thuật toán
        conflicted = csp.conflicted_vars(current) # Xét current
        if not conflicted: # Nếu current thỏa mản không vi phạm
            return current # Current là solution 
        var = random.choice(conflicted) # nếu vi phạm, sẽ chọn ngẫu nhiên một biến vi phạm, sau đó tiếp tục thuật toán
        val = min_conflicts_value(csp, var, current) # Chọn vị trí mà biến vi phạm ít nhất, lấy giá trị tại chỗ vi phạm ít nhất
        csp.assign(var, val, current) # gán vào asignment, nếu chưa phải solution sẽ chạy tiếp các biến khác 
    return None
